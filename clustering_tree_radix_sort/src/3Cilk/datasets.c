...
/*-------------------------(Εγραφή δεδομένων(συντεταγμένων) σε αρχείο)--------------------------*/
...
void cube(float *X, int N){//Παραγωγή τυχαίων αριθμών και εισαγωγή σε πίνακα
  ...
  for(int i=0; i<N; i++){
    for(int j=0; j<DIM; j++){
      X[i*DIM + j] = (float)rand() / (float) RAND_MAX;//Παραγωγή τυχαίων αριθμών(που αντιστοιχούν στις συντεταγμένες) και εισαγωγή στον x[]
    }
  }
